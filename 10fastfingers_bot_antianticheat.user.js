// ==UserScript==
// @name         10fastfingers AntiAntiCheat
 ...
 * - Inspired by https://gist.github.com/kecebongsoft/1510198
 */
...
(function antiAntiCheat10fastFingers() {
    'use strict';
    ...
    var isOnTypingTestPage = $('#reload-box').length !== 0;

    var bgUrls = [
        'https://i.pinimg.com/originals/08/eb/5b/08eb5b663e03a6831ed45950f08c1385.gif',
        'https://media.giphy.com/media/A06UFEx8jxEwU/giphy.gif',
    ];
    var bgUrlsSuccess = [
        'http://images4.fanpop.com/image/photos/22500000/Neo-in-The-Matrix-Reloaded-the-matrix-22575477-560-300.gif',
        'https://i.kym-cdn.com/entries/icons/original/000/021/807/4d7.png',
        'https://media.moddb.com/images/mods/1/31/30173/Moddb_thumg.png',
        'https://cdn190.picsart.com/230925670045202.gif',
        'https://media1.tenor.com/images/b23a908ae01021bc1064937bad061b11/tenor.gif',
    ];
    [].__proto__.__defineGetter__('random', function () {
     ...
     * @returns {Promise} resolve is called when the typing is complete
     */
    var typeIt = function (words = window.words, options = {}) {
        options = $.extend({
            wpm: 250,
            inputField: '#word-input',
        }, options);

        if (isNaN(options.wpm)) options.wpm = 250;
        if (!options.words || !options.words.length) options.words = $('#row1 > span').toArray().map(x => x.innerText);

        console.log('typeIt:', words);
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                unsafeEval(function (words, options, resolve) {
                    ...
                    timeout = setTimeout(hackit, interval_per_word);

                }, words, options, resolve);
    ...
    var antianticheat = function () {
        ...
        function fillText(result) {
            ...
            return new Promise(function (resolve, reject) {
                try {
                    $('#word-input').val(words.join(' ')); // type the words in the textfield
                    resolve();
                } catch (e) {
                    reject(e);
                }
                ...
                $('button#submit-anticheat').click();
            });
        }

        /**
         * recognizes the image and extracts the info and text using Tesseract OCR
         * @returns {Promise} -
        ...
        function recognize() {
            ...
            return getImageData($('#word-img img')[0]).then(imageData => Tesseract.recognize(imageData, {
                tessedit_char_whitelist: ',.' + 'abcdefghijklmnopqrstuvwxyz' // + 'ABCDEFGHIJKLMNOPQRSTUVWXY'
            }).progress(function (p) {
                $('#hackit-status-div').html(JSON.stringify(p, null, 4).replace(/\n+/g, '<br>'));
                debug && console.log('progress', p);
            }).then(function (result) {
        ...
        /**
         * @async
         * @param {HTMLImageElement} bimg
         * @returns {Promise}
         */
        function getImageData(bimg) {
            var b64url = getBase64Image(bimg);
            debug && console.log('base64url:', b64url);
    ...
    // Add HACKERMAN background
    // NOTE: this is extremely important or the script won't work!!
    function addBg() {
        $('body > div.container-modified').ready(function () {
            $('body > div.container-modified').css({
                'background-image': 'url("' + bgUrls.random + '")',
                'background-position': 'center center',
            });
        });
    }

    if (isOnTypingTestPage) {
        // injecting the hackit button and the inputfield
        $('#input-row > div').first()
            .append( // appending speed inputfield
                ...
                $('<div>')
                    ...
                    .append($('<span>').text('input words per minute'))
            )
            .append(
                // create hackit button
                $('<button>')
                    .attr({id: 'hackit-btn'})
                    .css({
                        'width': '100%',
                    ...
                    .click(function (e) { //
                        ...
                        var words = $('#row1 > span').toArray().map(x => x.innerText);
    ...
    } else if (isOnAnticheat) {
        addBg();
        // modify "Start" button
        $('#start-btn').css({width: 'auto'}).text('Start (without H@CK!NG)');

        $('#on-top')
            .css({ // gives a COOL HACKER THEME
                'background': '#000000',
                'color': '#00FF00',
                'font-family': 'monospace',
                'height': '100%',
